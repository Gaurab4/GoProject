...
const ImageList = ({ images }) => {
  ...
  return (
    <>
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(460px, 1fr))', gridGap: '15px', ml: 5, pt: 2 }}>
        {images[0].url.map((image, index) => (
          <Card key={index} hoverable sx={{ maxWidth: 1030, height: 650, borderRadius: 0, boxShadow: 0 }}>
            <CardMedia sx={{ cursor:'zoom-in', height: 680, width: 400 }} image={image} onClick={() => handleImageClick(image)} />
          </Card>
        ))}
      </Box>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="lg" >
        <DialogContent onClick={handleClose} sx={{ p: 0 ,cursor:'zoom-out'}}>
          ...
          <img src={selectedImage} alt="Enlarged" style={{ width: '100%', height: '110vh', objectFit: 'contain' }} />
        </DialogContent>
      </Dialog>
    </>
  );
